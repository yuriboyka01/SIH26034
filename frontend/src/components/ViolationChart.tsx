...
export default function ViolationChart({ violations }: { violations: ViolationSummaryItem[] }) {
  ...
  return <div className="space-y-5">{violations.map((violation) => {
    ...
    return <div key={violation.rule_id}><div className="mb-2 flex items-baseline justify-between gap-4"><div className="min-w-0"><p className="font-mono text-[11px] font-bold text-[#a9c7ff]">{violation.rule_id}</p><p className="mt-0.5 truncate text-sm text-[var(--text-muted)]">{violation.rule_name}</p></div><span className="font-mono text-sm font-semibold text-[var(--text)]">{violation.count}</span></div><div className="h-1.5 overflow-hidden bg-[#1c2635]"><div className="h-full bg-[#d77b76]" style={{ width: `${percentage}%` }} /></div></div>;
  })}</div>;
}
