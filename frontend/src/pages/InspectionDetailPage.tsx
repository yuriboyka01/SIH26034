...
export default function InspectionDetailPage() {
  ...
  return <div className="space-y-6"><Link to="/inspections" className="inline-flex items-center gap-1.5 text-sm text-[var(--text-muted)] hover:text-[var(--brand-hover)]"><ArrowLeft size={16} /> Back to inspection register</Link>
...
function AnalysisProgress({ phase, progress, imageCount, processedImageCount }: { phase: AnalysisPhase; progress: number; imageCount: number; processedImageCount: number }) {
  ...
  return <section className="app-surface overflow-hidden" aria-live="polite"><div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4"><div className="flex items-start gap-3"><Loader2 size={19} className="mt-0.5 shrink-0 animate-spin text-[var(--brand)]" /><div><p className="text-sm font-semibold text-[var(--text)]">{current.title}</p><p className="mt-1 text-xs leading-relaxed text-[var(--text-muted)]">{current.description}</p></div></div><div className="text-right"><span className="block font-mono text-sm font-semibold text-[var(--brand-hover)]">{progress}%</span><span className="font-mono text-[11px] text-[var(--text-muted)]">{processedImageCount} / {imageCount} images processed</span></div></div><div className="h-1.5 bg-[#182131]"><div className="h-full bg-[var(--brand)] transition-[width] duration-500" style={{ width: `${progress}%` }} /></div><ol className="grid gap-2 px-5 py-3 text-xs sm:grid-cols-3">{steps.map((step) => <li key={step.phase} className={`flex items-center gap-2 ${order[step.phase] < order[phase] ? 'text-[var(--success)]' : step.phase === phase ? 'text-[var(--brand)]' : 'text-[var(--text-faint)]'}`}><span className={`grid h-5 w-5 place-items-center rounded-full border text-[10px] ${order[step.phase] < order[phase] ? 'border-[var(--success)] bg-[var(--success)]/10' : step.phase === phase ? 'border-[var(--brand)] bg-[var(--brand)]/10' : 'border-[var(--line-strong)]'}`}>{order[step.phase] < order[phase] ? '✓' : order[step.phase]}</span>{step.label}</li>)}</ol></section>;
}
...
function EvidenceItem({ image, result, pendingDelete, onAskDelete, onCancelDelete, onConfirmDelete }: { image: InspectionImage; result: AnalysisResponse['images'][number] | null; pendingDelete: boolean; onAskDelete: () => void; onCancelDelete: () => void; onConfirmDelete: () => void }) {
  return <section className="app-surface overflow-hidden"><header className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--line)] px-4 py-3"><div className="min-w-0"><p className="truncate text-sm font-semibold text-[var(--text)]">{image.original_filename}</p><p className="mt-1 font-mono text-[10px] uppercase tracking-wide text-[var(--text-faint)]">{image.image_type} · {(image.file_size / 1024).toFixed(0)} KB</p></div>{pendingDelete ? <div className="flex items-center gap-2"><span className="text-xs text-[var(--danger)]">Remove this evidence?</span><button className="ui-button-danger !min-h-8 !px-2 !text-xs" onClick={onConfirmDelete}>Delete</button><button className="ui-icon-button" onClick={onCancelDelete} aria-label="Cancel delete"><X size={15} /></button></div> : <button className="ui-icon-button hover:!text-[var(--danger)]" onClick={onAskDelete} aria-label={`Delete ${image.original_filename}`}><Trash2 size={16} /></button>}</header><div className="p-4">{result ? <OCRResultsPanel imageUrl={image.url} result={result} /> : <div className="overflow-hidden rounded-md border border-[var(--line)] bg-[var(--surface-raised)]"><img src={image.url} alt={image.original_filename} className="max-h-[520px] w-full object-contain" loading="lazy" /><div className="border-t border-[var(--line)] px-4 py-3"><p className="text-sm font-semibold text-[var(--text)]">Evidence ready for analysis</p><p className="mt-1 text-xs text-[var(--text-muted)]">Run analysis to extract OCR declarations and evaluate applicable rules.</p></div></div>}</div></section>;
}
